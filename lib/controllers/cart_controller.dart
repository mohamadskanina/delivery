...
class CartController extends GetxController {
  ...
  void addItem(ProductModel product, int quantity) {
    var totalQuantity = 0;
    if (_items.containsKey(product.id ?? 0)) {
      ...
      if (quantity > 0) {
        ...
        Get.snackbar("Item Count", "you at least add an item in the cart",
            backgroundColor: const Color(0xFF89dad0),
            colorText: const Color(0xfff6f6f6));
      }
    }
  ...
  set setCart(List<CartModel> items) {
    ...
    for (int i = 0; i < storageItems.length; i++) {
      _items.putIfAbsent(
          storageItems[i].product!.id ?? 0, () => storageItems[i]);
    }
  }
